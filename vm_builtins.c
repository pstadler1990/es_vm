...
#define BEGIN_API_FUNC	do {\
    uint32_t stack_before = vm->stack.top;\
    printf("begin %d\n", arglen);\

#define END_API_FUNC    printf("end %d\n", vm->stack.top);\
                        } while(0);
...
uint32_t e_ext_my_external_func(e_vm* vm, uint32_t arglen) {
	...
	if(arglen > 0) {
		...
		e_stack_status_ret a2 = e_api_stack_pop(&vm->stack);
		if(a1.status == E_STATUS_OK && a1.val.argtype == E_NUMBER) {
			if(a2.status == E_STATUS_OK && a2.val.argtype == E_NUMBER) {
				// Push a1.value * a2.value onto stack
				printf("1: %f 2: %f\n", a1.val.val, a2.val.val);
				e_api_stack_push(&vm->stack, e_create_number(a1.val.val - a2.val.val));
				return 1;
			}
		}
	}
